...
Public Class Form1
    ...
    ' *** NOUVELLE VARIABLE : Référence au formulaire de verrouillage sur le PC serveur ***
    ...
    Private Sub Form1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        ' CheckBox1 est déjà gérée par le concepteur
    ...
    Private Sub HandleClient(client As TcpClient, token As CancellationToken)
        ...
        Try
            ...
            If Not String.IsNullOrEmpty(receivedMessage) Then
                ...
                ' *** Nouvelle logique : Traitement des commandes spécifiques ***
                ...
                ElseIf receivedMessage.Equals("COMMAND:TOGGLE_LOCKED_FORM", StringComparison.OrdinalIgnoreCase) Then
                    ToggleLockedForm()
                Else
                    ' Traitement standard du message (notification)
                    ShowBalloonNotification("Message Réseau Reçu", receivedMessage, ToolTipIcon.Info)
                End If
    ...
    ' *** Nouvelle fonction pour ouvrir/fermer le formulaire de verrouillage ***
    ...
    Private Sub btnSendNotification_Click(sender As Object, e As EventArgs) Handles btnSendNotification.Click
        ...
        Task.Factory.StartNew(Sub() SendMessageToServer(serverIpAddress, messageToSend))
    End Sub
    ...
    Private Sub btnLocked_Click(sender As Object, e As EventArgs) Handles btnLocked.Click
        ...
        ' Envoyer la commande de toggle du formulaire verrouillé
    ...
    Private Sub SendMessageToServer(serverIpAddress As String, message As String)
        ...
        Try
            ...
            If Not message.StartsWith("START_APP:", StringComparison.OrdinalIgnoreCase) AndAlso
               Not message.Equals("COMMAND:TOGGLE_LOCKED_FORM", StringComparison.OrdinalIgnoreCase) Then
        ...
        Else
            Try
                If CheckBox1.Checked Then
                    ...
                    LogMessage("Notification affichée en MessageBox : " & title & " - " & text, Color.Orange)
                Else
                    If NotifyIcon1.Icon Is Nothing Then
                    ...
                    LogMessage("Notification bulle envoyée : " & title & " - " & text, Color.DarkGreen)
                End If

            Catch ex As Exception
                LogMessage("Erreur lors de l'affichage de la notification (fallback au MessageBox) : " & ex.Message, Color.Red)
                MessageBox.Show(text, title & " (Erreur de notification)", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End Try
        End If
    End Sub
    ...
    Private Sub ExitApplication(sender As Object, e As EventArgs)
        ' Assurez-vous de fermer le LockedForm si l'application est quittée
        If Not IsNothing(lockedFormInstance) AndAlso Not lockedFormInstance.IsDisposed Then
            lockedFormInstance.Close()
            lockedFormInstance.Dispose()
            lockedFormInstance = Nothing
    ...
    Private Sub Form1_FormClosing(sender As Object, e As FormClosingEventArgs) Handles Me.FormClosing
        ' Assurez-vous de fermer le LockedForm si le formulaire principal est fermé
        If Not IsNothing(lockedFormInstance) AndAlso Not lockedFormInstance.IsDisposed Then
            lockedFormInstance.Close()
            lockedFormInstance.Dispose()
            lockedFormInstance = Nothing
